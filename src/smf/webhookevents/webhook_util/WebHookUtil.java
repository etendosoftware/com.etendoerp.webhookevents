...
public class WebHookUtil {
  ...
  public static void sendEvent(Webhook hook, BaseOBObject bob) throws Exception {
    ...
    String postJsonData = generateDataParametersJSON(hook.getSmfwheStdparamList(), bob);
     ...
     * System.out.println("Post Data : " + postJsonData); System.out.println("Response Code : " +
     * responseCode);
     * 
     * BufferedReader in = new BufferedReader(new InputStreamReader(con.getInputStream())); String
     * output; StringBuffer response = new StringBuffer();
     * 
     * while ((output = in.readLine()) != null) { response.append(output); } in.close();
     * 
     * // printing result from response System.out.println(response.toString());
     */
  }
